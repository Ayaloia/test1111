...
(() => {
    ...
    Game_Interpreter.prototype.addChoices = function(params, index, data, d) {
        this.checkChoiceConditions(params[0], data, d);
        if (params[1] !== -1) data.cancelType = params[1] + d;
    ...
    Game_Interpreter.prototype.checkChoiceConditions = function(texts, data, d) {
        const regIf = /\s?if\((.+?)\)/;
        const regEn = /\s?en\((.+?)\)/;
        ...
        for (;;) {
            ...
            if (command.indent === this._indent) {
                if (command.code === 402) {
                    ...
                    const helpTexts = this.getHelpTexts(index + 1);
    ...
    Game_Interpreter.prototype.getHelpTexts = function(index) {
        const commandNames = ['ChoiceHelp', '選択肢ヘルプ'];
        if (
            this._list[index].code === 108 &&
            commandNames.includes(this._list[index].parameters[0])
        ) {
            const texts = [];
    ...
    Game_Interpreter.prototype.choiceCancelType = function(data) {
        const cancelType = data.cancelType;
        if (cancelType.mod(10) === 8 || data.results.contains(cancelType)) {
            data.results.push(cancelType);
